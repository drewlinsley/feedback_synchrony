...
def build_model(settings):
	  ...
	  for time_step in range(num_steps):
	    ...
	    for layer in range(num_hidden_layers):
	      ...
	      if layer == target_layer:
	        ...
	        new_c = activation((tf.nn.conv2d(h_prev, Wc[layer], layer_strides, padding='SAME') + Wcb[layer]) + gated_prev_timestep)
	      else:
	...
	return session, merged, saver, optim, writer


def batch_train(session, merged, saver, optim, writer, X_train_raw, y_train_temp, settings):
	locals().update(settings)
	#Consider clipping
